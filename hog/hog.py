...
def perfect_square(score):
    if pow(score, 0.5) % 1 == 0.0:
        return True
    return False
def next_perfect_square(score):
    return int(pow(pow(score, 0.5) + 1, 2))
...
def play(strategy0, strategy1, update,
    ...
    """while score0 > goal and score1 > goal:
        if update == square_update:
            score0 = square_update(take_turn(strategy0, score1, dice))
            score1 = square_update(take_turn(strategy1, score0, dice))
        else:
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    if tail_strategy(score, opponent_score, threshold, num_rolls) == 0:
        temp = square_update(0, tail_points(opponent_score) + score, opponent_score)
    else:
        temp = square_update(0, score, opponent_score)
