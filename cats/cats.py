...
def about(topic):
    ...
    def func(para):
        for x in topic: 
            if x in split(lower(remove_punctuation(para))):
                return True
        return False
    return func
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        a = []
        for x in word_list:
            a = a + [diff_function(typed_word, x, limit)]
    
    if min(a) > limit:
        return typed_word
    else:
        return word_list[min_index(a)]
...
def feline_fixes(typed, source, limit):
    ...
    if limit == 0:
        if source == typed:
            ...
            return 1000
    if len(source) == 1 or len(typed) == 1:
        ...
        if source[0] == typed[0]:
            return 0 + feline_fixes(source[1:], typed[1:], limit)
        else:
...
def minimum_mewtations(start, goal, limit):
    ...
    2
    >>> minimum_mewtations("purng", "purring", big_limit)   # purng -> purrng t-> purring
    2
    ...
    if start == goal:  # Fill in the condition
        ...
        substitute = 1 + minimum_mewtations(start[1:], goal[1:], limit - 1)# is another recusive call 
        # BEGIN
    ...
    def progress(t, p):
        ...
        for i in range(len(t)):
            if t[i] == p[i]:
                ...
                return x/len(p)
        return len(t)/len(p)
    temp = progress(typed, prompt)
    dict = {'id' : user_id, 'progress' : temp}
    upload(dict)
    return temp
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    return match(words, [[times_per_player[player][i + 1] - times_per_player[player][i] for i in range(len(words))] for player in range(len(times_per_player))])
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for word in word_indices:
        ...
        for player in player_indices:  
            if minn > time(match, player, word):
                minn = time(match, player, word)
                best_player = player
        lst[best_player].append(get_word(match, word))          
    return lst
